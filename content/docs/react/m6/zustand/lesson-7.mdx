...
```jsx
// ❌ Bad: Shows nothing, confusing UI
function FavoritesPage() {
...
**No manual refresh needed!** Store updates → Component re-renders → UI updates.
...
function FavoritesPage() {
  const [favorites, setFavorites] = useState([]);
  // Separate state! Out of sync with HomePage! ❌
}
```
...
function HomePage() {
  const favorites = useListingsStore((state) => state.favorites);
  // Same favorites as FavoritesPage! ✅
}

function FavoritesPage() {
  const favorites = useListingsStore((state) => state.favorites);
  // Same favorites as HomePage! ✅
}
```
...
- ✅ **Reused ListingCard** - Same component in HomePage and FavoritesPage
