...
// Problem: Doesn't update on browser back/forward! ❌

// ✅ Good: Use router state
const location = useLocation();
<Link className={location.pathname === '/' ? 'active' : ''} />
...
<span className="badge">{favoritesCount}</span>
// Shows: Favorites [0] ← Looks bad!

// ✅ Only show when > 0
{favoritesCount > 0 && <span className="badge">{favoritesCount}</span>}
// Shows: Favorites [5] ← Looks good!
// Shows: Favorites ← When 0 (clean!)
```
...
// Every component needs to receive and pass props! ❌
```

**With Zustand:**

```jsx
// ✅ Good: Components read directly from store
function App() {
  return (
    <div>
...
// No props! Components are independent! ✅
```
...
toggleFavorite(5)
// ↓
// Navbar: Badge updates (3 → 4)
// HomePage: Button changes (🤍 → ❤️)
// FavoritesPage: List grows (adds listing 5)
```

**All automatic!** No manual synchronization needed. ✨
