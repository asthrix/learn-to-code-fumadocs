...
// How to filter by search? By price? ❌
...
**Option 1: Store filtered items in state (❌ Bad)**
```js
...
**Option 2: Compute on demand (✅ Good)**
```js
const useListingsStore = create((set, get) => ({
  items: [],
...
// Results:
// Item 1: ✅ matches "house", ❌ price too high → EXCLUDED
// Item 2: ❌ doesn't match "house" → EXCLUDED
// Item 3: ❌ doesn't match "house" → EXCLUDED
// Result: []
```
