...
Let's test and understand how favorites work! ❤️
...
**Let's test it and understand how it works!** 🧪
  ...
  return (
    <div style={{ padding: '2rem', border: '2px solid #0369a1', margin: '1rem' }}>
      ...
      <div style={{ marginTop: '1rem' }}>
        <p>Is Listing 1 favorited? {favorites.includes('1') ? '✅ Yes' : '❌ No'}</p>
        <p>Is Listing 5 favorited? {favorites.includes('5') ? '✅ Yes' : '❌ No'}</p>
        <p>Is Listing 9 favorited? {favorites.includes('9') ? '✅ Yes' : '❌ No'}</p>
      </div>
...
1. Click "Toggle Listing 1" → Array becomes `["1"]`
2. Click "Toggle Listing 5" → Array becomes `["1", "5"]`
3. Click "Toggle Listing 9" → Array becomes `["1", "5", "9"]`
4. Click "Toggle Listing 1" again → Array becomes `["5", "9"]`
...
// 2a. If YES (already favorited) → Remove it
state.favorites.filter(favId => favId !== id)
// Example: [1, 5, 9].filter(favId => favId !== 5)
// Result: [1, 9]

// 2b. If NO (not favorited) → Add it
[...state.favorites, id]
// Example: [...[1, 9], 5]
// Result: [1, 9, 5]
```
...
// Favorite automatically shows new price! ✅
...
```js
// ❌ Bad: Mutation (Zustand won't detect change)
toggleFavorite: (id) => {
  const state = get();
  ...
  set({ favorites: state.favorites });  // Same reference, no re-render! ❌
}

// ✅ Good: Immutable (creates new array)
toggleFavorite: (id) => set((state) => ({
  favorites: state.favorites.includes(id)
...
const arr = [1, 2, 3];
arr.push(4);  // Still the same reference! ❌

// New array creates new reference
const arr2 = [...arr, 5];  // New reference! ✅
```
...
function ListingsList() {
  ...
  // Does NOT re-render when favorites change ❌
...
- Component A: ✅ Re-renders (uses favorites)
- Component B: ❌ Doesn't re-render (uses items)
- Component C: ❌ Doesn't re-render (uses nothing)
...
function FavoriteButton({ listingId }) {
  ...
  return (
    <button onClick={() => toggleFavorite(listingId)}>
      {favorites.includes(listingId) ? '❤️' : '🤍'}
    </button>
  );
}
...
// toggleFavorite() → 100 re-renders ❌

// Good: All select favorites
// toggleFavorite() → 100 re-renders (but necessary!)

// Best: Mix of specific selectors
// - 10 components select favorites → 10 re-renders
// - 90 components select items → 0 re-renders
// toggleFavorite() → only 10 re-renders ✅
```
...
1. Each card has a favorite button (🤍)
2. Click the button → it turns red (❤️)
3. Favorites array updates: `[1, 5, 9]`
4. Click again → it turns white (🤍)
5. Favorites array updates: `[5, 9]`
6. Button animates with heartbeat effect! 💓

**All cards share the same favorites state!** ✨
...
4. All components selecting favorites re-render:
   - PropertyCard (button changes to ❤️)
...
function PropertyCard({ listing }) {
  ...
  // ✅ Available in FavoritesPage
  // ✅ Can persist to localStorage
  // ✅ Can show count in navbar
...
**✅ Lesson Complete!** Favorites functionality is working!
</Callout>

## Key Takeaways

- ✅ **toggleFavorite works** - Add/remove listings from favorites
- ✅ **ID-based storage** - Efficient and simple
- ✅ **Immutable updates** - Creates new arrays for re-renders
- ✅ **Selective re-renders** - Only components using favorites re-render
- ✅ **Shared state** - All components see same favorites
- ✅ **PropertyCard enhanced** - Favorite button with animation
- ✅ **Better than local state** - Global, persistent, shareable
- ✅ **Simpler than Redux** - No dispatch, no Provider, just hooks
