...
```
http://localhost:5173/listing/1   → Shows "Listing ID from URL: 1"
http://localhost:5173/listing/42  → Shows "Listing ID from URL: 42"
http://localhost:5173/listing/999 → Shows "Listing ID from URL: 999"
```
...
if (id === 42) {
  // ❌ Never true! Comparing string to number
}
...
// ✅ Works fine - APIs accept strings
...
// ❌ Doesn't work - rendered outside Router
function App() {
...
- ✅ Imported and used `useParams` hook
- ✅ Extracted `id` from URL parameters
- ✅ Displayed the ID in the UI
- ✅ Tested with different URLs
- ✅ Understood parameter types (always strings)
...
In **Lesson 7**, we'll use the `id` to **fetch the actual listing data** from the API! We'll implement loading states, error handling, and display real data based on the URL parameter. This is where everything comes together! 🚀
