...
prerequisites: ["/docs/react/m5/3_use-memo"]
learning_objectives:
...
<Accordions type='single' className='mt-4'>
   <Accordion title='Solution Walkthrough'>
      <p>Use `useCallback` so child components can skip re-rendering when data hasn’t changed.</p>
      ...
      ## Callback Results
      - `TaskCard` renders per toggle: 4 → 1
      - `ProjectSidebar` renders per filter: 3 → 1
      ```
...
<div className='mt-8 flex justify-between'>
   ...
   <a
      href='/docs/react/m5/5_custom-hooks'
