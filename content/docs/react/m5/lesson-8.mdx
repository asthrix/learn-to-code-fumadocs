...
- Type "b" → HomePage: 45ms (Orange)
  - ListingList: 38ms (Orange)
    - ListingCard × 1000: 0.038ms each (Red aggregate)
...
- Type "b" → HomePage: 12ms (Yellow)
  ...
  - ListingList: 10ms (Yellow)
    - ListingCard × 200: 0.05ms each (Green - only new cards)
...
| ListingCard (each) | Always renders | Only new ones | 80% fewer |
