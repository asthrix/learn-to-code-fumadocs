...
prerequisites: ["/docs/react/m3/4_use-ref-hook"]
learning_objectives:
...
-  **GET `/api/tasks`** → list tasks (supports status filter, search query).
-  **POST `/api/tasks`** → create a task.
-  **PATCH `/api/tasks/:id`** → update title or completion.
...
-  **Components** should stay declarative—consume hooks/services, don't fetch
   directly.
...
<Accordions type='single' className='mt-4'>
   <Accordion title='Solution Walkthrough'>
      ...
      ## API Contract
      - GET `/api/tasks` → `{ data: TaskDto[] }`
      - POST `/api/tasks` → `{ data: TaskDto }`
      - PATCH `/api/tasks/{id}` → `{ data: TaskDto }`
...
<div className='mt-8 flex justify-between'>
   <a
      href='/docs/react/m3/4_use-ref-hook'
