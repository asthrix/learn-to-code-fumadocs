...
# Lesson 1: Introduction to useEffect
...
```jsx title="❌ Problem: Side effects during render"
function Component() {
...
```jsx title="✅ Solution: useEffect controls when side effects run"
// [!code word:useEffect]
...
function ListingCard({ listing }) {
  useEffect(() => {
    console.log('ListingCard mounted:', listing.id);
    
    return () => {
      console.log('ListingCard unmounting:', listing.id);
    };
  }, [listing.id]);
...
<Accordions type="single">
  <Accordion id="rule-1" title="1. Only Call Hooks at the Top Level">
    ...
    ```jsx title="❌ Wrong: Conditional useEffect"
    ...
    ```jsx title="✅ Correct: Condition inside useEffect"
    function Component({ shouldFetch }) {
      useEffect(() => {
        if (shouldFetch) {
          fetchData();
  ...
  <Accordion id="rule-3" title="3. Include All Dependencies">
    ...
    ```jsx title="❌ Wrong: Missing dependency"
    ...
    ```jsx title="✅ Correct: All dependencies included"
    function Component({ userId }) {
      const [user, setUser] = useState(null);
...
```jsx title="❌ Wrong: useEffect for derived state"
...
```jsx title="✅ Correct: Calculate during render"
function Component({ items }) {
...
```jsx title="❌ Wrong: useEffect for event handling"
function Component() {
...
```jsx title="✅ Correct: Direct event handler"
function Component() {
  const handleClick = () => {
    alert('Clicked!');
  };
...
```jsx title="❌ Wrong: useEffect for initial state"
function Component({ userId }) {
  const [user, setUser] = useState(null);
...
```jsx title="✅ Correct: Initialize in useState"
function Component({ userId }) {
    ...
    <Accordions type="single">
      <Accordion id="answer-2" title="Show Fixed Code">
        ...
        **Fixes:**
        ...
        1. ✅ Store interval ID and clear it in cleanup
        2. ✅ Use updater function `prev => prev + 1` to avoid stale closure
        3. ✅ Empty dependency array ensures single interval
      </Accordion>
    </Accordions>
  </Step>
</Steps>
