...
<Accordion id='colors' title='Colors' value='colors'>
   ```jsx className="bg-white text-gray-900" className="text-yellow-500" ``` -
   `bg-white` - Background color white - `text-gray-900` - Text color dark gray
   - `text-yellow-500` - Text color yellow (for stars)
</Accordion>

{" "}
<Accordion id='typography' title='Typography' value='typography'>
   ```jsx className="text-lg font-semibold" ``` - `text-lg` - Font size large
   (1.125rem) - `text-sm` - Font size small (0.875rem) - `font-semibold` - Font
   weight 600 - `font-bold` - Font weight 700
</Accordion>

{" "}
<Accordion id='effects' title='Effects & Transitions' value='effects'>
   ```jsx className="rounded-xl shadow-md hover:shadow-xl transition-shadow" ```
   - `rounded-xl` - Border radius 0.75rem - `shadow-md` - Drop shadow -
   `hover:shadow-xl` - Larger shadow on hover - `transition-shadow` - Smooth
   shadow animation - `duration-300` - Animation duration 300ms
</Accordion>
...
<Accordions type='single'>
   <Accordion
      id='chal1'
      title='Challenge 1: Add Dark Mode Toggle'
      value='chal1'
   >
...
<Accordion id='chal2' title='Challenge 2: Add a Favorite Button' value='chal2'>
   Add a heart icon to each property card: ```jsx
   ...
   <Accordion
      id='chal3'
      title='Challenge 3: Add Loading Skeleton'
      value='chal3'
   >
