...
{" "}
<Step>
   ### Number Props Use curly braces for numbers: ```jsx
...
{" "}
<Step>
   ### Variable Props Use curly braces for variables: ```jsx const propertyTitle
   = "Beach House";
...
<Accordions type='single'>
   <Accordion id='props' title='Props (What We Just Learned)' value='props'>
      - **Passed from parent** to child - **Read-only** (child can't modify
      them) - Used to configure a component - Like function parameters ```jsx
      <PropertyCard price={250} /> // Parent passes // PropertyCard receives and
      displays ```
   </Accordion>

   <Accordion id='state' title='State (Coming in Module 2)' value='state'>
      - **Managed within** a component - **Can be modified** by the component -
      Causes re-renders when changed - Used for interactive data ```jsx const
      [count, setCount] = useState(0); // Component owns and can change this ```
   </Accordion>
</Accordions>
