---
title: "L5: PropertyList with Array.map()"
description:
...
```jsx title="Manual Repetition (Not Scalable)"
...
```javascript title="Array.map() Basics"
const numbers = [1, 2, 3];
...
```jsx title="Mapping Data to Components"
const properties = [
   { id: 1, title: "House" },
   { id: 2, title: "Loft" },
];
...
<Files>
   <Folder name='src'>
      <Folder name='components' defaultOpen>
         ...
         <File name='PropertyList.jsx ← CREATE THIS FILE' />
      </Folder>
   </Folder>
</Files>
...
```jsx title="Map Function Breakdown"
{
   properties.map((property) => (
      <PropertyCard
         key={property.id}
...
<Steps>
   ...
   <Step>
      ### (property) =>
      Each item is called `property` (you can name it anything)
   </Step>
   <Step>
      ### Return JSX
      For each property, return a `PropertyCard` component
   </Step>
   <Step>
      ### Pass Props
      Extract data from `property` object and pass as props
   </Step>
</Steps>
...
<Accordions type='single'>
   <Accordion id='key-rules' title='Rules for Keys' value='key-rules'>
      ...
      2. **Should be stable** (don't use array index if items can move)
      3. **Should be the same** across re-renders
      
      ```jsx title="Key Best Practices"
      // ✅ Good: Unique IDs
      key={property.id}
   ...
   <Accordion id='key-warning' title='What Happens Without Keys?' value='key-warning'>
      If you forget `key`, React shows this warning:
      
      ```
      Warning: Each child in a list should have a unique "key" prop.
      ```
      
      Your app will still work, but React can't optimize updates efficiently. Always include keys!
   </Accordion>
</Accordions>
...
App
 └─ HomePage
     └─ PropertyList
         ...
         ├─ PropertyCard (Downtown Loft)
         ├─ PropertyCard (Mountain Cabin)
         └─ PropertyCard (Lakefront Villa)
```
...
<Accordions type='single'>
   <Accordion id='missing-return' title='Forgetting to Return JSX' value='missing-return'>
      ```jsx title="Common Return Mistakes"
      // ❌ Wrong: No return, cards won't show
      {properties.map((property) => {
         <PropertyCard title={property.title} />;
      })}
   ...
   <Accordion id='missing-key' title='Forgetting the Key Prop' value='missing-key'>
      ```jsx title="Key Prop Required"
   ...
   <Accordion id='index-key' title='Using Index as Key (When List Changes)' value='index-key'>
      ```jsx title="Index as Key Considerations"
      // ⚠️ Risky if list can be reordered/filtered
...
<Accordions type="single">
  <Accordion id="chal1" title="Challenge 1: Add More Properties" value="chal1">
    ...
    ```jsx title="Adding More Properties"
    {
      id: 5,
      title: "Urban Studio",
  ...
  <Accordion id="chal2" title="Challenge 2: Filter High-Rated Properties" value="chal2">
    ...
    ```jsx title="Filtering Properties"
    <PropertyList 
...
-  ✅ Understanding and implementing the `key` prop
-  ✅ Creating container components (PropertyList)
...
-  ✅ PropertyList.jsx exists in `src/components/`
-  ✅ PropertyList is imported and used in HomePage
...
<div className='mt-6 flex gap-4'>
   <Button variant='outline' asChild>
      <a href='/docs/react/m1/lesson-4'>← Previous: PropertyCard Component</a>
   </Button>
   <Button asChild>
      <a href='/docs/react/m1/lesson-6'>Next: Styling with Tailwind →</a>
   </Button>
</div>
