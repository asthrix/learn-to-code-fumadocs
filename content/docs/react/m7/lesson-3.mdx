...
2. If valid token exists → Get user data
3. If no token → User stays signed out
...
Access Token:
  ...
  → Expires every 15 min

Refresh Token:
  → Used rarely (only when access expires)
  → Long-lived = better UX
  → Secure storage = hard to steal
```

**Flow:**

```
1. Sign in → Get both tokens
2. Access token in memory, refresh token in cookie
3. Use access token for requests
4. When access expires → Use refresh to get new access
...
4. But refresh token still in cookie ✅
5. User reopens app
6. We use refresh token to get new access token
7. User still signed in! 🎉
```

**Without token refresh:**

```jsx
// ❌ User has to sign in every time
User closes browser
  → Memory cleared
  → No session check
  → Shows sign-in page
...
Cookie: refreshToken=abc123xyz  ← Sent automatically
```
