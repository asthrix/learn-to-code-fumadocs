...
Wrong order:
  path="*" matches first → 404 page ❌

Correct order:
  path="/sign-in" matches → SignInPage ✅
```
...
```
User visits:
  /            → HomePage (matches first)
  /favorites   → FavoritesPage (matches second)
  /sign-in     → SignInPage (matches third)
  /unknown     → NotFoundPage (no match, falls to *)
  /random/path → NotFoundPage (no match, falls to *)
```
...
// 3. No typos
<Route path="/signin" />   // ❌ No hyphen
<Route path="/sign-in" />  // ✅ Has hyphen

// 4. Case sensitivity (usually lowercase)
<Route path="/SignIn" />   // ❌ May not match
<Route path="/sign-in" />  // ✅ Standard
```
...
- ✅ **Routes map URLs to components** (`/sign-in` → `SignInPage`)
