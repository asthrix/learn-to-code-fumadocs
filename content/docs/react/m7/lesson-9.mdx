...
// 1. Correct endpoint
api.post('/auth/login', ...)  // ✅
api.post('/login', ...)       // ❌ Wrong path

// 2. Correct payload format
{ email: "...", password: "..." }  // ✅
{ username: "...", pwd: "..." }    // ❌ Wrong fields
...
const { user } = useAuth();  // ✅
const user = null;           // ❌ Not using context

// 3. Conditional rendering checks user
{user && <Navbar />}  // ✅
{true && <Navbar />}  // ❌ Always shows

// 4. State updates before navigate
setUser(userData);
navigate('/');  // ✅ State set first

navigate('/');
setUser(userData);  // ❌ Too late
```
