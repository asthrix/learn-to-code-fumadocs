...
prerequisites: ["/docs/react-new/m3/5_introduction-data-fetching"]
learning_objectives:
...
<Accordions type='single' className='mt-4'>
   <Accordion title='Solution Walkthrough'>
      ...
      ## API Client
      - `fetchJson(path, options)` → wraps fetch with defaults
      - `createApiClient()` → returns get/post/patch/delete helpers
      - Errors → `ApiError` with status and body
      ```
...
<div className='mt-8 flex justify-between'>
   <a
      href='/docs/react-new/m3/5_introduction-data-fetching'
      className='text-sm font-medium text-muted-foreground hover:text-foreground'
   >
      ← Introduction to Data Fetching
   </a>
   <a
      href='/docs/react-new/m3/7_data-fetching-part-2'
      className='text-sm font-medium text-primary hover:text-primary/80'
   >
      Next · Data Fetching Part 2
   </a>
</div>
