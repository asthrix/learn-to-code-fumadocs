...
prerequisites: ["/docs/react-new/m3/7_data-fetching-part-2"]
learning_objectives:
...
````ts filename="src/lib/api/cache.ts"
   ...
   export async function retryWithBackoff<T>(operation: () => Promise<T>, attempts = 3) {
      let delay = 200;
...
TaskFlow Pro now caches task queries, performs optimistic updates, and retries transient failures to keep the UI smooth.
