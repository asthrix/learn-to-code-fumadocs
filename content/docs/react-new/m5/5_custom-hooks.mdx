...
prerequisites: ["/docs/react-new/m5/4_use-callback"]
learning_objectives:
...
**Why:** Consumers shouldn’t know whether data is memoised or fetched from
cache.
...
<Accordions type='single' className='mt-4'>
   <Accordion title='Solution Walkthrough'>
      ...
      ## Custom Hooks
      - `useProjectTasks(projectId)` → returns memoised tasks + counts
      - `useTaskFilters()` → exposes `status`, `search`, setters, and URL sync
      ```
...
<div className='mt-8 flex justify-between'>
   <a
      href='/docs/react-new/m5/4_use-callback'
   ...
   >
      Next · Custom Hooks Best Practices
   </a>
</div>
